-- SQL Server Sandbox Init Script
-- Creates readonly user and sample tables for testing
-- Note: This script should be run after container starts

-- Create sandbox database
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'sandbox_db')
BEGIN
...
IF NOT EXISTS (SELECT * FROM sys.server_principals WHERE name = 'sandbox_user')
BEGIN
    CREATE LOGIN sandbox_user WITH PASSWORD = 'Sandbox123!';
END
GO

IF NOT EXISTS (SELECT * FROM sys.database_principals WHERE name = 'sandbox_user')
BEGIN
